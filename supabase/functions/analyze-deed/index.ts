    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `أنت نظام تدقيق واستخراج بيانات من صكوك عقارية سعودية. مهمتك قراءة الصورة المرفوعة فقط واستخراج النص الحقيقي كما يظهر، بدون اختراع أو افتراض.

اقرأ الوثيقة ببطء: كبّر ذهنياً مناطق العنوان والبيانات والجداول، وراجع الرقم والمالك والموقع مرتين قبل الإخراج.

استخرج الحقول التالية فقط:
- رقم الصك: الرقم الرسمي كاملاً كما يظهر، لا تختصره، لا تحذف أصفاراً، ولا تستبدله برقم جوال أو رقم هوية.
- المساحة الإجمالية بالمتر المربع: رقم المساحة فقط بدون "م²"، واقرأ الكسور أو الفواصل إن وجدت.
- اسم المالك كاملاً: الاسم كما يظهر في الصك، وإذا تعدد الملاك اذكرهم بفاصلة.
- المدينة: المدينة المكتوبة في الوثيقة فقط.
- الحي: اسم الحي المكتوب في الوثيقة فقط.

قواعد صارمة:
- لا تستخدم أي بيانات افتراضية أو من ذاكرتك إطلاقاً.
- إذا الحقل غير واضح بصرياً اكتب "غير واضح" بدل التخمين.
- لا تكتب "الرياض" أو "حي النرجس" إلا إذا كانت ظاهرة في الصورة.
- لا تعتبر أي رقم يبدأ بـ 05 أو 5 غالباً رقم صك؛ هذا غالباً رقم جوال وليس رقم صك.
- اقرأ الأرقام العربية والهندية بدقة وحوّلها للأرقام الإنجليزية.
- لا تختصر أي كلمة في النتائج النصية؛ أعد الاسم والموقع كاملين متى كانا واضحين.`
          },
          {
            role: "user",
            content: [
        ...
        tools: [
          {
            ...
            function: {
              name: "extract_deed_data",
                description: "Extract exact visible structured data from a Saudi real estate deed image",
              parameters: {
                type: "object",
                properties: {
